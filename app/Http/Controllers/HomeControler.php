...
class HomeControler extends Controller
{
	...
	public function UpDatePost($log_user_id,$post_id,Request $request)
	{
		...
		$UpDatePost = User_Post::where('id',$post_id)->update(['header' => $request['new_head'],'body' => $request['new_body']]);
		return redirect("/HomePage/" . $log_user_id . "/MyPosts");
	}
	...
	public function CommentsPost(Request $request,$log_user_id,$post_id)
	{
		...
		$user_name = User::where("id",$log_user_id)->first()['name'];
		$comment = new Comment();
	...
	public function RegisterValidation(Request $request)
	{
		...
		$user->save();
		return rediecrt("/HomePage/" . $user->id);
	}
